<template>
  ...
  const buttonClass = computed(() => (props.isAvailable ? 'bg-green-500 text-white px-4 py-1 mt-2 rounded-lg hover:bg-green-700 shadow-lg' : 'bg-gray-500 text-white px-4 py-1 mt-2 rounded-lg cursor-not-allowed shadow-lg'));
  
  const opcionPrestar = () => emit('open-modal'); // Emit the event directly
